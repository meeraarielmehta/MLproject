...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        intermed_y = nn.ReLU(nn.AddBias(xw1, self.b1))
        predicted_y = nn.AddBias(nn.Linear(intermed_y, self.w2), self.b2)
        return predicted_y
    ...
    def train(self, dataset):
        ...
        for x, y in dataset.iterate_forever(self.batch_size):
            ...
            grad_wrt_w1, grad_wrt_w2, grad_wrt_b1, grad_wrt_b2 = nn.gradients(loss_node, [self.w1, self.w2, self.b1, self.b2])
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        # Initialize your model parameters here
        "*** YOUR CODE HERE ***"
        self.w1 = nn.Parameter(self.num_chars, 275)
        self.hidden_w = nn.Parameter(275, 275)
        self.final_w = nn.Parameter(275, len(self.languages))
        self.b = nn.Parameter(1, len(self.languages))
    ...
    def run(self, xs):
        ...
        for i in range(1, len(xs)):
            h = nn.Add(nn.Linear(xs[i], self.w1), nn.Linear(h, self.hidden_w))

        predicted_y = nn.AddBias(nn.Linear(nn.ReLU(h), self.final_w), self.b)
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(self.batch_size):
                loss_node = self.get_loss(x, y)
                grad_wrt_w1, grad_wrt_hidden_w, grad_wrt_final_w, grad_wrt_b = nn.gradients(loss_node, [self.w1, self.hidden_w, self.final_w, self.b])
                (self.w1).update(grad_wrt_w1, self.multiplier)
                (self.hidden_w).update(grad_wrt_hidden_w, self.multiplier)
                (self.final_w).update(grad_wrt_final_w, self.multiplier)
